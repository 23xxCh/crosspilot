...
def _row_images(payload: dict, mapping: dict[str, dict], index: int, audit_images: list[dict] | None=None) -> list[dict]:
    ...
    def image_record(*, role: str, role_key: str, url: str, position: int) -> dict:
        ...
        return {'role': role, 'role_key': role_key, 'position': position, 'url': url, 'local_path': (mapping.get(url) or {}).get('path', ''), 'download_ok': bool((mapping.get(url) or {}).get('ok')), 'source': audit.get('source') or 'source', 'source_url': audit.get('source_url') or '', 'source_local_path': (mapping.get(audit.get('source_url')) or {}).get('path', '') if audit.get('source_url') else '', 'assessment': assessment, 'decision': audit.get('decision') or '', 'evidence': audit.get('evidence') or assessment.get('evidence') or ''}
    images = []
...
def build_quarantine_rows(quarantine_products: list[dict], mapping: dict[str, dict], *, row_offset: int) -> list[dict]:
    ...
    for index, item in enumerate(quarantine_products):
        ...
        for position, image in enumerate(item.get('images') or []):
            ...
            images.append({'role': {'main': '主图', 'variant': '变种图', 'attachment': '附图'}.get(image.get('role'), str(image.get('role') or '图片')), 'role_key': image.get('role') or 'attachment', 'position': position, 'url': url, 'local_path': (mapping.get(url) or {}).get('path', ''), 'download_ok': bool((mapping.get(url) or {}).get('ok')), 'source': image.get('source') or 'source', 'source_url': image.get('source_url') or '', 'source_local_path': (mapping.get(image.get('source_url')) or {}).get('path', '') if image.get('source_url') else '', 'assessment': assessment, 'decision': image.get('decision') or '', 'evidence': image.get('evidence') or assessment.get('evidence') or ''})
        bullets = source_row.get('bullets') or []
        rows.append({'row': row_offset + index + 1, 'product_id': str(item.get('product_id') or ''), 'title': str(source_row.get('title') or item.get('title') or ''), 'description': str(source_row.get('description') or ''), 'bullets': [str(bullets[i] or '') if i < len(bullets) else '' for i in range(5)], 'keywords': str(source_row.get('keywords') or ''), 'images': images, 'quarantined': True, 'quarantine_reasons': item.get('reasons') or []})
    return rows
from pathlib import Path
...
def export_review(input_path: str | Path, output_dir: str | Path, *, translate_workers: int=30, download_workers: int=32, audit_by_product: dict[str, list[dict]] | None=None, quarantine_products: list[dict] | None=None, shared_cache_dir: str | Path | None=None, translation_cache_path: str | Path | None=None, run_id: str | None=None, run_metrics: dict | None=None) -> dict:
    ...
    (output_dir / '终审包.html').write_text(render_html(rows, summary), encoding='utf-8')
    return summary
