...
- 图片下载只允许公网 HTTP(S) 地址；重定向逐跳复查，流式限制大小和像素，禁止访问本机或内网。
...
Agent 入口会再次独立读取输入并比对正式输出；终审包、审核数据、异常清单或 `generation_requests=0` 任一验收失败，都必须生成失败结果，不能只相信流水线返回值。
...
- 失败时上一版正式表未被覆盖。
- Windows 占用“最新”目录时使用带恢复副本的事务发布；逐文件失败必须自动恢复旧版。
