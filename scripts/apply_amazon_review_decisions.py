...
from scripts.pipelines.amazon_image_safety import validate_image_url
...
def _backup(
    ...
    backup_root = formal_path.parent / "审核应用备份" / stamp
    backup_root.mkdir(parents=True, exist_ok=False)
    formal_backup = backup_root / formal_path.name
    shutil.copy2(formal_path, formal_backup)
    result = {"formal_json": str(formal_backup)}
    if review_package and review_package.exists():
        package_backup = backup_root / "终审包"
        shutil.copytree(
            review_package,
            package_backup,
            copy_function=_copy_or_link,
        )
        result["review_package"] = str(package_backup)
    return result


def _central_override_path(formal_path: Path) -> Path:
    for parent in formal_path.parents:
        if parent.name == "亚马逊表":
            return parent / "检查图片文字" / "图片人工覆盖.json"
    return formal_path.parent / "检查图片文字" / "图片人工覆盖.json"
...
def _update_review_package(
    ...
    *,
    ...
    data_path = package_dir / "中文文案检查表.json"
    ...
    (package_dir / "中文文案检查表.html").write_text(
        render_html(review_data["products"], summary),
        encoding="utf-8",
    )
    _atomic_json(
        package_dir / "审核决定_已应用.json",
        application_report,
    )
...
def apply_decisions(
    ...
    *,
    ...
    overrides_path = formal_path.with_name(
        formal_path.stem + "_图片人工覆盖.json"
    )
    existing_overrides = (
    ...
    existing_overrides["overrides"].extend(
        false_positive_overrides
    )
    _atomic_json(overrides_path, existing_overrides)
    ...
    report = {
        ...
        "false_positive_overrides": str(overrides_path),
    ...
    report_path = formal_path.with_name(
        formal_path.stem
        + "_审核应用报告_"
...
def main(argv: list[str] | None = None) -> int:
    ...
    return cli_main(["apply", *arguments])
