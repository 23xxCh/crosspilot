"""Fail-closed orchestration for Amazon image safety."""
...
from .remediation import cached_generation, generate_safe_replacements
...
def _review_source_images(
    ...
    *,
    ...
    print(
        f"结构化图片初审 {len(missing)} 张（"
        f"{AMAZON_REVIEW_CONCURRENCY} 并发，自适应退避）...",
        flush=True,
    )
...
def _confirm_intrinsic_brand_images(
    urls: list[str],
    *,
    assessments: dict[str, dict[str, Any]],
    ...
    confirm_missing = [
        url for url in intrinsic_urls if url not in confirmations
    ]
    ...
    _, confirm_stats = adaptive_map(
        confirm_missing,
        confirm_one,
        operation="amazon_high_risk_confirmation",
        initial_workers=min(
            AMAZON_REVIEW_CONCURRENCY,
...
def run_structured_image_safety_gate(
    ...
    runtime_metrics = (
        runtime_metrics if isinstance(runtime_metrics, dict) else {}
    )
    ...
    def effective_assessment(
        ...
        override = manual_overrides.get(
            (str(product_id), str(role), str(url))
        )
        if override:
    ...
    _confirm_intrinsic_brand_images(
        urls,
        assessments=assessments,
    ...
    for url in urls:
        ...
        for item in usage[url]:
            ...
            records[item["product_id"]].append(
                assessment_record(
                    url=url,
                    role=item["role"],
        ...
        if not assessment_is_intrinsic_brand(first):
            continue
        second = confirmations.get(url) or unknown_image_assessment(
            "second confirmation unavailable"
        )
    ...
    # Risky/unknown attachments never reach the formal output.
    ...
    for product_id, row in row_by_id.items():
        ...
        kept = [
            ...
            if assessment_status(
                effective_assessment(product_id, "attachment", url)
            )
            == "safe"
        ]
        ...
        if _audit_text(before) != _audit_text(kept):
            _add_audit(
                ...
                before,
                kept,
                method="structured_vision",
                reason="drop_risk_or_unknown_attachment",
                severity="warning",
                action="终审确认被删除附图的风险标签与证据",
            )
    ...
    for url in sorted(urls):
        ...
        if status == "risk" and not assessment_is_intrinsic_brand(
            assessments.get(url)
        ):
            if "main" in roles:
            ...
            for item in usage[url]:
                ...
                if (
                    ...
                    continue
                quarantine[item["product_id"]].append(
                    {
                        "code": f"unknown_{item['role']}_image",
                        "role": item["role"],
                        "url": url,
                        "message": "主图/变种图无法可靠判断，按高安全策略隔离",
                        "assessment": assessments[url],
                    }
                )

    targets_to_generate = generate_safe_replacements(
        remediation_targets,
        cache=cache,
        cache_path=cache_path,
    ...
    for product_id, row in row_by_id.items():
        ...
        main_status = assessment_status(
            effective_assessment(product_id, "main", main)
        )
        if main_status == "risk":
            ...
            elif not assessment_is_intrinsic_brand(
                effective_assessment(product_id, "main", main)
            ):
                failed_main += 1
                quarantine[product_id].append(
                    {
                        "code": "main_image_remediation_failed",
                        "role": "main",
                        "url": main,
                        "message": "风险主图生成失败或生成后复审仍有风险",
                        "assessment": assessments.get(main),
                        "generation_failure": gen_failures.get(f"main:{main}"),
                    }
                )
        ...
        for source_url in before_variants:
            ...
            if replacement:
                ...
                remediated_variants.append(source_url)
                if not assessment_is_intrinsic_brand(
                    effective_assessment(
                        product_id,
                        "variant",
                        source_url,
                    ...
                    quarantine[product_id].append(
                        {
                            "code": "variant_image_remediation_failed",
                            "role": "variant",
                            "url": source_url,
                            "message": "风险变种图生成失败或生成后复审仍有风险",
                            "assessment": assessments.get(source_url),
                            "generation_failure": gen_failures.get(
        ...
        row["var_img"] = (
            remediated_variants[0] if remediated_variants else ""
        )
        ...
        quarantine_records.append(
            {
                ...
                "reasons": reasons,
                "images": row.get("_image_assessments") or [],
                "source_row": {
                    "title": str(row.get("title") or ""),
                    "description": str(row.get("desc") or ""),
                    "bullets": [
                        str(bullets[index] or "")
    ...
    runtime_metrics["image_confirmations"] = {
        url: dict(value) for url, value in confirmations.items()
    }
    runtime_metrics["image_remediation"] = {
        "reviewed": len(assessments),
        "flagged": status_counts["risk"],
        "clean_retained": status_counts["safe"],
        "unknown_retained": 0,
        "attachment_reviewed": sum(
            1
            for url, items in usage.items()
            if any(item["role"] == "attachment" for item in items)
        ),
        "attachment_flagged": sum(
            1
            for url, items in usage.items()
            if any(item["role"] == "attachment" for item in items)
            and assessment_status(assessments.get(url)) != "safe"
        ),
        "attachment_deleted": attachment_deleted,
        "generated_main": generated_main,
        "generated_variant": generated_variant,
        "failed_main": failed_main,
        "failed_variant": failed_variant,
        "generation_url_checked": len(targets_to_generate),
        "generation_url_valid": generated_main + generated_variant,
        "generation_url_invalid": failed_main + failed_variant,
    }
    if quarantine_records:
        quality_issues.append(
            f"图片安全门隔离 {len(quarantine_records)} 个商品，"
            "未写入正式回填表"
        )
    save_cache(cache_path, cache)
    print(
        "图片安全门完成: "
        f"安全 {status_counts['safe']}，风险 {status_counts['risk']}，"
        f"未知 {status_counts['unknown']}，删附图 {attachment_deleted}，"
        f"隔离商品 {len(quarantine_records)}",
        flush=True,
    )
    if progress:
        progress(1, 1)
